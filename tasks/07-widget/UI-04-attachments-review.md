...
## Подготовительная реализация D3 (проверки отложены)

В worktree `/private/tmp/hackalem-ui01.NF4q1E`, ветка `codex/ui-01-client-transport`,
добавлены локальная панель загрузки, стадии обработки, построчная review с
version guard и переход выбранных строк в отдельное предложение UI-03.
Демо-проверка заголовка файла и синтетические строки **не являются разбором
загруженного документа**. Исходные байты не отправляются и не сохраняются.
Подробнее: `frontend/README.md`, раздел UI-04.

До контракта ATT-01…04 не подключены server capability/limits, настоящий upload,
OCR/vision, matching, source ACL и атомарная версия review между вкладками.
Форматы и конфликтные сценарии в Compose/browser не проверены. Команды
build/lint/test/browser не выполнялись по просьбе пользователя. Commit/PR: нет;
статус остаётся `todo`, чекбоксы приёмки не отмечены.
