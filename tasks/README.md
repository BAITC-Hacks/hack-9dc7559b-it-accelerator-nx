...
Поле `acceptance_depends_on`, где указано, добавляет условия итогового прогона и закрытия карточки: каталог/вложения ждут настоящую identity, OPS-02 — все модули продукта, QA-01 — recovery/лимиты, QA-02 — agent pipeline/runner. Оно не блокирует раннюю реализацию на согласованных контрактах и подготовку тестов. Обе группы связей показаны в [графе](DEPENDENCIES.md).
